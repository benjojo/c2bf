...
{
    /* FIXME: the declaration specifiers are currently ignored, since all data
     * types have the same width
     * they will be important when pointer arithmetic is supported */
    if (a->has_init_declarator_list) {
        genbf_init_declarator_list(a->v2);
    }
}
